...
class HiveService {
  ...
  static Future<void> _addSampleBookmarks() async {
    final sampleBookmarks = [
      ...
      HiveBookmark(
        ...
        arabicText:
            'أَصْبَحْنَا وَأَصْبَحَ الْمُلْكُ لِلَّهِ ۖ وَالْحَمْدُ لِلَّهِ ۖ لَا إِلَٰهَ إِلَّا اللَّهُ وَحْدَهُ لَا شَرِيكَ لَهُ ۖ لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ وَهُوَ عَلَىٰ كُلِّ شَيْءٍ قَدِيرٌ',
        translation:
  ...
  static Future<void> deleteBookmark(String bookmarkId) async {
    final index = _bookmarksBox.values.toList().indexWhere(
      (b) => b.id == bookmarkId,
    );
    if (index != -1) {
  ...
  static HiveUserProgress getProgress() {
    final progress = _progressBox.get('default');
    if (progress == null) {
      final newProgress = HiveUserProgress();
      _progressBox.put('default', newProgress);
      return newProgress;
    }
    return progress;
  }
  ...
  static Future<void> updateLastReadPosition(
    int surahNumber,
    String surahName,
    int verse,
  ) async {
